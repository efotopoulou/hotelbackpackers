...
$mensaje->setDatos($output);
}else if($year && $month && !$week && $limit) {
$output = topPlatillosMonth($tipoEstadistica,$year,$month,$limit,$estadisticas);
$mensaje->setDatos($output);	
}else if($year && $month && $week && $limit){
$output = topPlatillosWeek($tipoEstadistica,$year,$month,$week,$limit,$estadisticas);
$mensaje->setDatos($output);	
}else if($year && !$month && !$week && $limit){
$output = topPlatillosYear($tipoEstadistica,$year,$limit,$estadisticas);
$mensaje->setDatos($output);	
}
...
if ((sizeof($lineasCMM))>0){
		...
		$strCaja .= "<set name='" . $i . "' value='" .$lineasCMM[$i]->suma . "' color='".colorMonth($i)."' link='".urlencode("http://localhost:8081/restbar/view.php?page=historicocajaRest&date=".$date). "' />";
		...
		for($i=0;$i<sizeof($weekdays);$i++){
		    ...
		    if ($linea){
			 foreach($linea as $key1) {
		        ...
		        $strCaja[$n] .= "<set value='" .$key1->suma . "' color='".colorweek($n)."' link='".urlencode("http://localhost:8081/restbar/view.php?page=historicocajaRest&idcaja=".$key1->id_caja). "'/>";
...
function topPlatillosWeek($tipoEstadistica,$year,$month,$week,$limit,$estadisticas){
$date = $year."-".$month."-".$week;
	if (($week+6)>daysOfMonths($month,$year)) $minas = monthsDescription($month+1);
	else $minas = monthsDescription($month);

  $lineasTP = $estadisticas->topPlatillosWeek($tipoEstadistica,$year,$month,$date,$limit);
...
function topPlatillosMonth($tipoEstadistica,$year,$month,$limit,$estadisticas){
$minas = monthsDescription($month);

  $lineasTP = $estadisticas->topPlatillosMonth($tipoEstadistica,$year,$month,$limit);
 if ((sizeof($lineasTP))>0){
...
function topPlatillosYear($tipoEstadistica,$year,$limit,$estadisticas){
  $lineasTP = $estadisticas->topPlatillosYear($tipoEstadistica,$year,$limit);
 if ((sizeof($lineasTP))>0){
