...
function loadstock($stock){
    ...
    if ((sizeof($stockbar))>0){
	  ...
	  $stockInfo[$i]=array("idBebida"=>$stockbar[$i]->idBebida,"numBebida"=>$stockbar[$i]->numBebida,"familia"=>$stockbar[$i]->familia,"nombre"=>$stockbar[$i]->nombre,"stockbar"=>$stockbar[$i]->stockbar,"unidadventa"=>$stockbar[$i]->unidadventa);
	  }
