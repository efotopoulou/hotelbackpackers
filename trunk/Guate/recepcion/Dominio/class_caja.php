...
class caja{
	...
	if($rs->getRecordCount()>0){ 
		...
		while($rs->next()){
		$result=$rs->getRow();
		$ors[$n] = new CajaComanda($result["idComanda"],$result["estado"],$result["fechaHora"],$result["total"],$result["efectivo"],$result["clientType"],$result["nombre"],$result["free"]);
		$n++;					
		}
    }else{
		$result=null;
...
//echo($rs->getRecordCount());
	if($rs->getRecordCount()>0){ 
		...
		while($rs->next()){
		$result=$rs->getRow();
		$ors[$n] =new usuariocomanda($result["idComanda"],$result["numComanda"],$result["estado"],$result["fechaHora"],$result["total"],$result["efectivo"],$result["clientType"],$result["nombre"]);
		$n++;			
		}
    }else{
		$result=null;
