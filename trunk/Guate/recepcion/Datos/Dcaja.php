...
class Dcaja{
	...
	const TOTAL_CUENTA = 'select sum(total) as total from(select sum(t4.total) as total from comanda t1,trabajador t3,comandacredito t4 where t1.idComanda=t4.idComanda and t1.id_cliente=t3.idTrabajador and t1.tipoCliente=5 and t3.idTrabajador=? and t4.cobrado=0 and t4.procedencia="HR" and t1.estado="credito" union select sum(t4.total) as total from restbar_bd.comanda t1,trabajador t3,comandacredito t4 where t1.idComanda=t4.idComanda and t1.id_cliente=t3.idTrabajador and t1.tipoCliente=5 and t3.idTrabajador=? and t4.procedencia="RB" and t1.estado="credito" union select sum(t1.dinero) as total from movimientocredito t1,movimiento t2 where t1.id_movimiento=t2.id_movimiento and t1.id_usuario=? and t1.procedencia="HR" union select sum(t1.dinero) as total from movimientocredito t1,restbar_bd.movimiento t2 where t1.id_movimiento=t2.id_movimiento and t1.id_usuario=? and t1.procedencia="RB") as total';
