...
function recargaEstadoRestaurante(){
 $.getJSON("jsongestionbar.php",{recarga:"yes"}, function(json){
...
var a=algo;
	...
	if (a=="family" && familiaid){
           if(confirm('�Estas seguro que quieres eliminar esta familia?')){
             $.getJSON("jsongestionbar.php",{familydeleteid:familiaid}, function(json){json = verificaJSON(json);loadfamilias(json);});
		   ...
		   if(confirm('�Estas seguro que quieres eliminar este platillo?')){
		     $.getJSON("jsongestionbar.php",{bebidadeleteid:platilloid}, function(json){json = verificaJSON(json);loadbebidas(json);});
   ...
   var color = "#"+$("#myhexcode").val();
    if(namefamily && color ){
    	if(confirm('�Estas seguro que quieres guardar esta familia?')){
	      $.getJSON("jsongestionbar.php",{namefamily:namefamily,color:color}, function(json){json = verificaJSON(json);loadfamilias(json);});
    	}
		$("#namefamily").val("");
		$("#myhexcode").val("");
		changedisplay('b4');
   ...
   var familiaid=$("#familiasTable .btnpress").attr("id");
	if(namefamilymod && familiaid){
	     if(confirm('�Estas seguro que quieres modificar esta familia?')){
	        $.getJSON("jsongestionbar.php",{namefamilymod:namefamilymod,famidmod:familiaid}, function(json){json = verificaJSON(json);loadfamilias(json);});
	     }
		 $("#namefamilymod").val("");
		 changedisplay('b4mod');
...
function guardarbebida(){
	...
	if(idbebida && namebebida && precioN && precioL && bebidaFamily){
        if(confirm('�Estas seguro que quieres guardar este platillo?')){
           $.getJSON("jsongestionbar.php",{idbebida:idbebida,namebebida:namebebida,precioN:precioN,precioL:precioL,bebidaFamily:bebidaFamily}, function(json){
           	  json = verificaJSON(json);
	...
	var precioLmod = $("#precioLmod").val();
	  if(platilloid && precioNmod && precioLmod){
         if(confirm('�Estas seguro que quieres modificar esta bebida?')){
             $.getJSON("jsongestionbar.php",{idbebidamod:platilloid,precioNmod:precioNmod,precioLmod:precioLmod}, function(json){json = verificaJSON(json);loadbebidas(json);});
         }
		 $("#idplatomod").val("");
		 $("#nameplatomod").val("");
		 $("#precioNmod").val("");
