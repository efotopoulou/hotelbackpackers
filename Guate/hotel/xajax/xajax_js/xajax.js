...
debugTag=this.debugWindow.document.getElementById('debugTag');if(!debugTag)
throw new Error();text=text.replace(/&/g,"&amp;");text=text.replace(/</g,"&lt;");text=text.replace(/>/g,"&gt;");debugTag.innerHTML=('<b>'+(new Date()).toString()+'</b>: '+text+'<hr/>')+debugTag.innerHTML;}catch(e){alert("Xajax Debug:\n "+text);}
...
else if(cmd=="as"){cmdFullname="addAssign/addClear";{eval("objElement."+property+"=data;");}
}
