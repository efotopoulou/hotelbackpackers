...
if ((sizeof($lineasCMM))>0){
		...
		$strCaja .= "<set name='" . $i . "' value='" .$lineasCMM[$i]->suma . "' color='".colorMonth($i)."' link='".urlencode("http://localhost:8080/recepcion/view.php?page=historicocaja&date=".$date). "' />";
		...
		for($i=0;$i<sizeof($weekdays);$i++){
		    ...
		    if ($linea){
			 foreach($linea as $key1) {
		        ...
		        $strCaja[$n] .= "<set value='" .$key1->suma . "' color='".colorweek($n)."' link='".urlencode("http://localhost:8080/recepcion/view.php?page=historicocaja&idcaja=".$key1->id_caja). "'/>";
