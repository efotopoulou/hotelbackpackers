...
class Destadisticas{
	...
	const TOP_PLATILLOS_MONTH = 'select t2.nombre,sum(t1.cantidad) as freq from lineacomanda t1,platillo t2,caja t3,comanda t4 where t3.id_caja=t4.id_caja and t4.idComanda=t1.idComanda and t1.idPlatillo=t2.idPlatillo and year(t3.fechaHoraApertura)=? and month(t3.fechaHoraApertura)=? group by t2.nombre order by freq desc limit ?';
	const TOP_PLATILLOS_WEEK ='select t2.nombre,sum(t1.cantidad) as freq from lineacomanda t1,platillo t2,caja t3,comanda t4 where t3.id_caja=t4.id_caja and t4.idComanda=t1.idComanda and t1.idPlatillo=t2.idPlatillo and year(t3.fechaHoraApertura)=? and month(t3.fechaHoraApertura)=? and weekofyear(t3.fechaHoraApertura)=weekofyear(?) group by t2.nombre order by freq desc limit ?';	
    const TOP_PLATILLOS_YEAR ='select t2.nombre,sum(t1.cantidad) as freq from lineacomanda t1,platillo t2,caja t3,comanda t4 where t3.id_caja=t4.id_caja and t4.idComanda=t1.idComanda and t1.idPlatillo=t2.idPlatillo and year(t3.fechaHoraApertura)=? group by t2.nombre order by freq desc limit ?';	
